...
let arrDept = [];
	...
	function getPaymentList(currentShowPageNo) {
		...
		$.ajax({
			url:"<%= ctxPath %>/admin/getPaymentList.pm",
	...
	function makePageBar(currentShowPageNo, startdate, enddate, minus_month) {
		...
		$.ajax({
			url:"<%= ctxPath %>/admin/paymentListTotalPage.pm",
			data:{"startdate": startdate,
				  "enddate":enddate,
				  "month_payment":minus_month,
				  "arrDept":arrDept},
		...
		Swal.fire({
			   ...
			   if (result.isConfirmed) { // 만약 모달창에서 confirm 버튼을 눌렀다면
				   ...
				   const month = minusGetMonth();
					...
					$.ajax({
						url:"<%= ctxPath %>/admin/checkedPayment.pm",
						type:"POST",
		...
		Swal.fire({
			   ...
			   if (result.isConfirmed) { // 만약 모달창에서 confirm 버튼을 눌렀다면
				   ...
				   const enddate = month+"-25";
					...
					$.ajax({
						url:"<%= ctxPath %>/admin/allPayment.pm",
						type:"POST",
						data:{"startdate": startdate,
							  "enddate":enddate,
							  "month_payment":minus_month},
...
<body>
    ...
    <div id="pay-stub-content">
        <div id="category" class="d-flex">
            <a href="<%= ctxPath%>/admin/payStub.pm" class="text-muted font-weight-bold mr-2 detail-category"><span>급여명세서 목록</span></a> <!-- border-bottom border-dark 을 사용하여 url에 따라 밑줄 생성 -->
