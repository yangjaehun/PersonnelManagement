...
<script>
  ...
  $(document).ready(function() {
        ...
        select: function(info) {
          ...
          initialDate: new Date(),
      	  events:function(info, successCallback, failureCallback) {
      		  ...
      		  $.ajax({
      			  url:"<%= ctxPath %>/schedule/selectScheduleList.pm",
				  dataType:"JSON",
      	  ...
      	  eventClick: function(info) {
	        		...
	        		*/
	        		$.ajax({
	        			url:"<%= ctxPath %>/schedule/selectDetailSchedule.pm",
	        			data:{"schedule_no" :schedule_no},
	        			dataType:"JSON",
	        			success:function(json) {
	        				
	        				if (json.redirect == "true") {
	  		                		window.location.href = "<%= ctxPath %>/schedule/calendar.pm";
	  		            		}
	        				else {
      ...
      // 공유자 추가하기
		$("input#joinuser").bind("keyup",function(){
			...
			//	console.log("확인용 joinUserName : " + joinUserName);
				$.ajax({
					url:"<%= ctxPath%>/schedule/insertSchedule/searchJoinUserList.pm",
      ...
      $("button#schedule_register").click(function () {
    			...
    			$.ajax({
    				url:"<%= ctxPath %>/schedule/insertSchedule.pm",
    				type:"POST",
    				...
    				success:function(json) {
    					...
    					if(json.n == 1) {
    						...
    						setTimeout("location.href='<%= ctxPath %>/schedule/calendar.pm'", 1000);
		   ...
		   Swal.fire({
			   ...
			   if (result.isConfirmed) { // 만약 모달창에서 confirm 버튼을 눌렀다면
				   const daterange = $("form[name='schedule_modify_delete'] input#daterange").val().trim()
					...
					$.ajax({
						url:"<%= ctxPath %>/schedule/updateSchedule.pm",
						type:"POST",
	   ...
	   $("button#schedule_delete").click(function() {
		   ...
		   Swal.fire({
			   ...
			   if (result.isConfirmed) { // 만약 모달창에서 confirm 버튼을 눌렀다면
				   ...
				   $.ajax({
						url:"<%= ctxPath %>/schedule/deleteSchedule.pm",
						type:"POST",
						data:{"schedule_no":schedule_no},
						dataType:"JSON",
						success:function(json) {
