...
<script>
	...
	function getAlarm(){
		...
		$.ajax({
	    	url : "<%=request.getContextPath()%>/alarm/getAlarmList.pm",
    		dataType: "JSON",
	...
	function getPastAlarm(){
		...
		$.ajax({
	    	url : "<%=request.getContextPath()%>/alarm/getPastAlarmList.pm",
    		dataType: "JSON",
			success: function(json){ 
				let html = "";
				if(json.length>0){
	...
	function readAlarm(alarmno){
		$.ajax({
	    	url : "<%=request.getContextPath()%>/alarm/readAlarm.pm",
	...
	function readAllAlarm(){
		$.ajax({
	    	url : "<%=request.getContextPath()%>/alarm/readAllAlarm.pm",
			success: function(){ 
				getAlarm();
				getPastAlarm();
			},
