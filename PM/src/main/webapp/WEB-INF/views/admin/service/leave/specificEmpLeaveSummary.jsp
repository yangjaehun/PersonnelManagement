...
<jsp:include page="specifiEmpLeaveCategory.jsp" />
...
<script type="text/javascript">
	...
	function getModaldata(pk_leave_type){
		$.ajax({
	    	url : "<%=ctxPath%>/leave/getLeaveDate.pm",
	...
	function requestLeaveForm() {
		...
		$.ajax({
	    	url : "<%=ctxPath%>/leave/requestLeave.pm",
	    	data : queryString,
	    	type: 'POST',
	...
	function getRecord(year){
		$.ajax({
	    	url : "<%=ctxPath%>/leave/getLeaveRecode.pm?empno=${empno}",
	    	data : {'year' : year},
	    	dataType: "JSON",
			success: function(json){
				...
				if(json.length>0){
					...
					$.each(json, function(index, item){	
						...
						if(item.opproval_status == 0){
							html += '<td class="tail"><button type="button" class="btn btn-outline-secondary btn-sm submitCancle">승인/반려</button></td>';
						} else {
	...
	function getPlan(year){
		$.ajax({
	    	url : "<%=ctxPath%>/leave/getLeavePlan.pm?empno=${empno}",
	    	data : {'year' : year},
	    	dataType: "JSON",
			success: function(json){
...
<!-- 휴가 상세 모달 -->
<%@ include file="modal/speccificEmpLeaveDetailModal.jsp" %>
