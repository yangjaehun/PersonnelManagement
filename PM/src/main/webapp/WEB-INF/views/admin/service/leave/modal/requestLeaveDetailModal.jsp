...
function openLeaveDetail(request_leaveno) {
	...
	$.ajax({
    	url : "<%=request.getContextPath()%>/leave/getLeaveDetail.pm",
					 	 ...
					 	 html+='</span>'+
					      	'<div class="box my-3 py-1">'+
					      		'<div class="inbox">'+
								 	...
								 	if(json.add_file ==1 && json.filename!=null){
						      			html+='<div class="ininbox"><div class="label">· 첨부파일</div><div class="list"><span class="mailFiles" onclick="javascript:location.href=\'<%=ctxPath%>/leave/downloadLeaveFile.pm?fileName='+json.filename+'&orgFilename='+json.orgfilename+'\'" >'+json.orgfilename+'</span></div></div>';
						      		}
...
function approvalRequestLevae( approval_status ,request_leaveno, empno){
	$.ajax({
    	url : "<%=request.getContextPath()%>/leave/approvalRequestLevae.pm",
    	data : {"request_leaveno" : request_leaveno,
...
function addLeavFile(){
	...
	$.ajax({
    	url : "<%=request.getContextPath()%>/leave/addLeavFile.pm",
    	data : queryString,
    	type: 'POST',
...
function deleteRequestLeave(request_leaveno) {
	...
	$.ajax({
    	url : "<%=request.getContextPath()%>/leave/deleteRequestLeave.pm",
    	data : {"request_leaveno" : request_leaveno},
		success: function(){
