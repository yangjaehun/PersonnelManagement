...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if( ${ not empty requestScope.empno }){
			...
			$.ajax({
		    	url : "<%=ctxPath%>/messenger/getEmpName.pm",
		    	data:{"empno": empno},
	    		dataType: "TEXT",
	    		async:true,
	...
	function readCheckedMsg(){
		...
		if(ck_msgno_list.length>0){
			...
			$.ajax({
		    	url : "<%=ctxPath%>/messenger/updateCheckedMsg.pm",
		    	type: 'POST',
	...
	function readAllMsg(){
		$.ajax({
	    	url : "<%=ctxPath%>/messenger/updateAllMsg.pm",
			success: function(){ 
				window.location.reload();
			},
			error: function(request, status, error){
	...
	function readMailContent(msgno){
		$.ajax({
	    	url : "<%=ctxPath%>/messenger/getMailContent2.pm",
	    	type: 'POST',
	    	data : {"msgno" : msgno},
	    	dataType: "JSON",
			success: function(json){
				...
				if(Number(json.having_attach) >0){
					$.ajax({
				    	url : "<%=ctxPath%>/messenger/getMailFile.pm",
							...
							if(json2.length>0){
								...
								$.each(json2, function(index2, item2){
									html+='<span style="font-size: 10pt; color: gray;"><i class="fas fa-solid fa-paperclip ml-3 mr-1"></i></span>'+
							        	'<span class="mailFiles" onclick="javascript:location.href=\'<%=ctxPath%>/messenger/downloadMailFile.pm?fileName='+item2.fileName+'&orgFilename='+item2.orgFilename+'\'" >'+item2.orgFilename+'</span><br>';
								});
	...
	function goSearch(){
		...
		location.href = "<%=ctxPath%>/messenger/receivedMessage.pm?searchWord="+searchWord;
	}
